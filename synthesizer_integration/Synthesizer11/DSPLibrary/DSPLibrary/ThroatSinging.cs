   ...
   public class ThroatSinging : WaveStream
    {
        ...
        [DllImport("Dll1.dll", CallingConvention = CallingConvention.Cdecl)]
        public static extern int throatSinging_init_left();
        [DllImport("Dll1.dll", CallingConvention = CallingConvention.Cdecl)]
        public static extern int throatSinging_init_right();
        [DllImport("Dll1.dll", CallingConvention = CallingConvention.Cdecl)]
        public static extern double get_throat_singing_tick_left();
        [DllImport("Dll1.dll", CallingConvention = CallingConvention.Cdecl)]
        public static extern double get_throat_singing_tick_right();
        [DllImport("Dll1.dll", CallingConvention = CallingConvention.Cdecl)]
        public static extern int throatSinging_push_sine_left(double amplitude, double frequency);
        [DllImport("Dll1.dll", CallingConvention = CallingConvention.Cdecl)]
        public static extern int throatSinging_push_sine_right(double amplitude, double frequency);
        ...
        public ThroatSinging()
        {
            ...
            //Initialize the dll
            throatSinging_init_left();
            throatSinging_init_right();
            ...
            for (int tone_i = 0; tone_i < 7; tone_i++)
            {
                throatSinging_push_sine_left(amplitudes_norm[tone_i], frequencies[tone_i]);
                throatSinging_push_sine_right(amplitudes_norm[tone_i], frequencies[tone_i]);
            }
        }
        ...
        public override int Read(byte[] buffer, int offset, int sampleCount)
        {
            ...
            for (int n1 = 0; n1 < sampleCount / 4; n1 += 2)
            {
                double temp4 = 0.0; 
                float temp = 0.0f;  
                for (int channel = 0; channel < 2; channel++)
                {
                    if(channel == 0)
                    {
                        temp4 = get_throat_singing_tick_left();
                        temp = (float)temp4;
                    }
                    else
                    {
                        temp4 = get_throat_singing_tick_right();
                        temp = (float)temp4;
                    }
                    byte[] bytes = BitConverter.GetBytes(temp);
                    //float randomFloat = (float)rand.NextDouble();
                    //byte[] bytes = BitConverter.GetBytes(randomFloat);
                    buffer[n1 * 4 + channel * 4 + 0] = bytes[0];
                    buffer[n1 * 4 + channel * 4 + 1] = bytes[1];
                    buffer[n1 * 4 + channel * 4 + 2] = bytes[2];
                    buffer[n1 * 4 + channel * 4 + 3] = bytes[3];
                }
                n2++;
